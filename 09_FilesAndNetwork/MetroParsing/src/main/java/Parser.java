...
public class Parser {
  ...
  private void generateStations(Document document)
      throws IllegalArgumentException {
    Elements tableMetro = document.select(cSS_QUERY_TO_BODY_TABLE_METRO)
        .nextAll("tr").not("tr.shadow");
    Elements tableCircle = document.select(cSS_QUERY_TO_BODY_TABLE_CIRCLE)
        .nextAll("tr");
    Elements tableMono = document.select(cSS_QUERY_TO_BODY_TABLE_MONO)
        .nextAll("tr");
  ...
  private void parseStationsFromElements(Elements elements) {
    ...
    while (iter.hasNext()) {
      ...
      if (station.isBlank()) {
        station = elTemp.selectFirst("tr > td:nth-child(2) > a").text();
      }
      String lineNumber = elTemp
          .selectFirst("tr > td:nth-child(1) > span:first-child").text();
      String secondLineNumber = elTemp
          .select("tr > td:nth-child(1) > span:nth-child(4)").text();
  ...
  private boolean addStationToResult(String station, String lineNumber) {
    ...
    result.getStations()
        .put(lineNumber, new ArrayList<>(Arrays.asList(stationArr)));
    return true;
  }

  private void parseConnections(String lineName, String lineNumber,
      Element el) {
    Element[] lineNums = el.select("tr>td:nth-child(4)")
        .not("td[data-sort-value=Infinity]")
        .select("span:nth-child(odd)").toArray(new Element[0]);
    Element[] lineNames = el.select("tr>td:nth-child(4)")
        .not("td[data-sort-value=Infinity]")
  ...
  private void chekAndChangeStationsInConnections()
  ...
  private boolean excludeInvalidObjects(List<Line> connections) {
    long elementsCount = connections.stream()
        .filter(
            conLine -> result.getStations().containsKey(conLine.getNumber()))
        .count();
    return elementsCount > 1;
  }

  private boolean changeStationName(String lineNum, String connectionName,
      Line line) {
