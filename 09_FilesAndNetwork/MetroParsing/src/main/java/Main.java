...
public class Main {
  ...
  private static final String JSON_FILE_OUTPATH = "src/main/resources/test.json";
  ...
  private static void printJson(Result object, String filePath)
      throws IOException {
  ...
  private static void printStationsCount(JsonObject object) {
    for (JsonElement jsonElement : object.getAsJsonArray("lines")) {
      ...
      int stationsCount = object.getAsJsonObject("stations")
          .getAsJsonArray(lineNum).size();
      int stringLength = 33 - lineName.length();
      String form = "%" + stringLength + "s";
      System.out.printf(
          "Количество действующих станций на линии " + form + " %s : %d\n", "",
