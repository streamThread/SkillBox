...
public class RedisStorage {
    ...
    User getRandomUser() {
        Long randomLong = new Random().longs(1L, (registeredUsers.size() + 1L)).findFirst().getAsLong();
        User user = registeredUsers.stream().filter(u -> u.getId().equals(randomLong)).findFirst().get();
